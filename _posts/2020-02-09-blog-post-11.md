...
If $\Vcal$ is a vector space, then its dual space $\Vcal^{\*}$ is also a vector space. We can again define the set of all linear transformations from $\Vcal^{\*}$ to $\Field$ as its dual space $\Vcal^{\*\*} = \mathscr{L}(\Vcal^{\*},\Field)$, which makes it the double dual space of $\Vcal$. Unlike $\Vcal^{\*}$ where we needed to define basis, there is an elegant way to go from $\Vcal$ to $\Vcal^{\*\*}$ via an isomorphism $\Psi$. If $x\in\Vcal$, and $\hat{x} = \Psi(x)\in\Vcal^{\*\*}$ acts on a linear functional $f\in\Vcal^{\*}$ and evaluates $f$ at $x$ returning an element in the field $\Field$, i.e.,
...
> **Theorem** - If $\Vcal$ is a vector space and is finite dimensional, then $\Psi : \Vcal\to\Vcal^{\*\*}$ is an isomorphism.
