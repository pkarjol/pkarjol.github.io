...
where $\mathcal{P}_s$ is the operator that projects its arguments to the $s$-sized $\ell_0$ ball. This can be easily done via sorting the elements based on the absolute values of the argument vector. The algorithm starts at $\zero$ and refines the solution at each step keeping only the $s$ dominant values in ite iterates.
