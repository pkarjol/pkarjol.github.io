...
> **Generalization error for Gaussian Ensemble** - If $\SIGMA_{ii} \leq 1$ and if $\xhat_s$ is the output of OMP after $s=\Theta\roundbrack{\kappa(\SIGMA)\log\kappa(\SIGMA) \sstar}$ iterations. If $\Sset = \supp{\xhat_s}$, then with high probability,
> 
> $$
