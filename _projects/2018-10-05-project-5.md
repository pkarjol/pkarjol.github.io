...
> **Result on T. Zhang's bound** - If $\xhat_s$ is the outout of OMP after $s=\Theta\roundbrack{\kappas{s}^2\log\kappa_s \sstar}$ iterations, then with high probability
> $$
> \inv{n}\esqnorm{\A(\x_s-\xbar)} \lesssim \inv{n}\sigma^2\sstar\kappas{s+\sstar}\log\kappa_{s+\sstar}\log d
> $$

Upon slight modifications and tightening T. Zhang's analysis, we proove a better generlaization bound that matches the fast rate upto logarithmic factors in $\kappa$ and $d$.
