...
where $$\x\in\Rd{d}$$, $$\sstar << d$$, the ambient dimension, $$\lznorm{\cdot}$$ is the $$\ell_0$$ norm that denotes the number of non-zero elements of its arguments, and $$f$$ is a real valued function.
...
### Lower bound matrix construction
...
We show that under the noiseless setting $(\sigma = 0)$, if $\epsilon<1-\inv{\sstar}$, OMP if run on $\M^\brackeps$, will continue including incorrect features untill only the correct ones are left.
