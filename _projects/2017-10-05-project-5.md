...
The Linear form of Sparse Linear Regression $$\roundbrack{\slr}$$ is a representative problem and results typically extend easily to the general case. We cosider the statistical setting of $$\roundbrack{\slr}$$ where the response is corrupted by adding a i.i.d. gaussian noise.

$$
\y = \A\x\xbar + \etavec = \A_\Sstar\xbar_\Sstar + \etavec
$$
where $$\etavec\sim\Normal{\zero,\I_n}$$ and $$\text{supp}(\xbar) = \sstar$$
