...
$$
2. **Restricted Isometry Property (RPI)** - Instead of enforcing an element-wise restcriction on the off-diagonals of the matrix $$\A^T\A$$, RIP requries a more general assumption that all $$s$$-sized principal block sub-matrices of $$\A^T\A$$ are $$\delta_s$$ close to $$\I_s$$ in the operator norm, i.e.,
$$
\enorm{\A_\Sset^T\A-\I_s} \leq \delta_s\\
\text{or, } (1-\delta_s)\esqnorm{\vvec} \leq \esqnorm{\A\vvec} \leq (1+\delta_s)\esqnorm{\vvec} \qquad \forall \Sset \text{ s.t. } \abs{\Sset}\leq s, \text{ and, } \forall \vvec \in\Rd{d}
$$
