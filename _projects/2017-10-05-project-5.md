...
1. *Incoherence* - Implies that the $$\abs{\inner{\A_i,A_j}}$$ should be bounded by some small positive real number $M$ for all $$i\neq j$$. In specific it can be shown OMP (which we will discuss in detail below) can recover $$\xbar$$ in ecactly $$\star$$ steps if $$M\leq \inv{2\sstar-1}$$.
