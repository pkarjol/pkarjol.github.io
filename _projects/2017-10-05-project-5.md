...
The plot describes how the error $$\enorm{\A\xhat_s-\y}$$ changes when we run IHT up to convergence at level $$s$$ (on the x-axis). The blue-dashed line represents the euclidean norm of the noise. We see that Accelerated IHT achieves lower generalization errors at an smaller value of $$s$$.
